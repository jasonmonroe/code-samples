...
class Candidate:
    ...
    def _donation_limits(self) -> tuple(float, float):
        ...
        if self.party in ["Democrat", "Republican"]:
            min_donation = 100 
            max_donation = CANDIDATE_DONATION_MAX
        elif self.party in ["Green", "Libertarian"]:
            max_donation = CANDIDATE_DONATION_MAX / 2
        elif self.party in ["Progressive", "Socialist of America"]:
            max_donation = CANDIDATE_DONATION_MAX / 2.5
        else:
